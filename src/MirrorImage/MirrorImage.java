...
class MirrorImage {
    ...
    static char[][] make_mirror(char[][] originalArray) {
        char[][] reversedArray = new char[4][13];

        for(int i = 0; i < 4; i++)
            for (int j = 0; j < 13; j++) {
                /*switch (originalArray[i][12 - j]) {
                    case '(' -> reversedArray[i][j] = ')';
                    case ')' -> reversedArray[i][j] = '(';
                    case '/' -> reversedArray[i][j] = '\\';
                    case '\\' -> reversedArray[i][j] = '/';
                    default -> reversedArray[i][j] = originalArray[i][12 - j];
                }*/
                if(originalArray[i][12 - j] == '(') reversedArray[i][j] = ')';
                else if (originalArray[i][12 - j] == ')') reversedArray[i][j] = '(';
                else if (originalArray[i][12 - j] == '/') reversedArray[i][j] = '\\';
                else if (originalArray[i][12 - j] == '\\') reversedArray[i][j] = '/';
                else reversedArray[i][j] = originalArray[i][12 - j];
            }
        return reversedArray;
    }

    static void main(String[] args) {
        char[][] forwardArr = make_forward();
        char[][] reverseArr = make_mirror(forwardArr);

        for(int i = 0; i < 4; i++,System.out.print("\n"))
            for (int j = 0; j < 13; j++)
                System.out.print(forwardArr[i][j]);
