        ...
        for ( int bulb = 1; bulb <= max; bulb++)
            ...
            for(int students = 1; students <= max; students++)
                if(bulb * students < hallway.size())
                    hallway.get(bulb * students).pullString();

        for (Lightbulb bulb : hallway)
            //Prints state of specific bulbs
            if (values.contains(hallway.indexOf(bulb)))
                System.out.println("Bulb " + hallway.indexOf(bulb) + " is " + (hallway.get(hallway.indexOf(bulb)).isOnOrOff() ? "On." : "Off."));
        scan.close();
    }
}